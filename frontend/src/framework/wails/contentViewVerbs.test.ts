...
describe("driving a native surface", () => {
  ...
  it("navigate carries the surface and the address", async () => {
    await wailsContentViewHost.navigate("brw-a", "https://example.org");
    expect(delivered).toEqual([{ id: "brw-a", message: { verb: "navigate", url: "https://example.org" } }]);
  });
  ...
  it("history carries the step itself, not a direction", async () => {
    ...
    await wailsContentViewHost.history("brw-a", -1);
    await wailsContentViewHost.history("brw-a", 1);
    await wailsContentViewHost.history("brw-a", -3);
  ...
  it("a step of zero is refused rather than sent as a verb nobody answers", async () => {
    await expect(wailsContentViewHost.history("brw-a", 0)).rejects.toThrow();
    expect(delivered).toEqual([]);
  });

  it("reload and stop carry nothing else", async () => {
    await wailsContentViewHost.reload("brw-a");
    await wailsContentViewHost.stop("brw-a");
  ...
  it("a refusal from the surface reaches the caller", async () => {
    ...
    refuse = new Error("native surface brw-a is not in the applied inventory at sequence 4");
    await expect(wailsContentViewHost.reload("brw-a")).rejects.toThrow(/inventory/);
  });
  ...
  it("what this host still does not do is refused by name", async () => {
    ...
    await expect(wailsContentViewHost.evalJs("brw-a", "1")).rejects.toThrow(/evalJs/);
  });
});
