...
describe("development unit source selection", () => {
  ...
  it("writes disable and enable to composition settings", async () => {
    await usePlugins.getState().reload();
    invoke.mockClear();
    invoke.mockImplementation(async (...input: unknown[]): Promise<unknown> => {
      const command = input[0] as string;
  ...
  it("keeps runtime state unchanged when enabled settings cannot be saved", async () => {
    await usePlugins.getState().reload();
    invoke.mockImplementation(async (...input: unknown[]): Promise<unknown> => {
      const command = input[0] as string;
      if (command === "settings_get") return { revision: 1 };
    ...
    const disable = await usePlugins.getState().disable(ID);
    expect(disable).toMatchObject({ ok: false, code: "INTERNAL" });
    expect(usePlugins.getState().plugins[ID].status).toBe("enabled");

    usePlugins.setState((state) => ({
      plugins: { ...state.plugins, [ID]: { ...state.plugins[ID], status: "disabled" } },
      enabledIds: [],
    }));
    const enable = await usePlugins.getState().enable(ID);
    expect(enable).toMatchObject({ ok: false, code: "INTERNAL" });
    expect(usePlugins.getState().plugins[ID].status).toBe("disabled");
