// Native window title = active project name — the in-app title bar is custom and does not draw this
...
// duplication (macOS convention is the document name alone). Only a window with no project (empty
// exception state) falls back to the app name (getName — the real name per identity).
// Updated on every active-project switch, rename, and tab add/remove (sessions subscription).
