// Recent workspace list — data source for the rail and the control-plane workspace map. core kv
// "recentWorkspaces" (makeCoreStore: app.data authority + localStorage cache + cross-window
// broadcast — every window shows the same list no matter which one opened the workspace).
// Recorded on explicit open success (addWorkspaceClaimed) and on default boot — restore does not
...
type Store = ReturnType<typeof makeCoreStore<RecentWorkspace[]>>;
...
const storeSlot = moduleState("state/recentWorkspaces#storeSlot.v", () => ({ v: null as Store | null }));
function recentStore(): Store {
  storeSlot.v ??= makeCoreStore<RecentWorkspace[]>({
    key: "recentWorkspaces",
    lsKey: "soksak.recentWorkspaces",
    fallback: [],
    ...coreStoreDeps,
  });
  return storeSlot.v;
...
export async function recordRecentWorkspace(root: string, alias: string): Promise<void> {
  try {
    const s = recentStore();
    ...
    console.warn("recording the recent workspace failed:", e);
  }
}

/** Lookup for the rail and the workspace map. */
/**
 ...
 */
export function asRecentWorkspaces(value: unknown): RecentWorkspace[] {
  if (!Array.isArray(value)) return [];
  return value.filter(
    (entry): entry is RecentWorkspace =>
      typeof entry === "object" && entry !== null && typeof (entry as RecentWorkspace).root === "string",
  );
}

export async function listRecentWorkspaces(): Promise<RecentWorkspace[]> {
  try {
    return asRecentWorkspaces(await recentStore().hydrate());
  } catch {
    return [];
  }
}
...
export async function removeRecentWorkspace(root: string): Promise<void> {
  try {
    ...
    console.warn("removing the recent workspace failed:", e);
  }
}

/** Reactive recent list (shared by rail and picker) — data-change(recentWorkspaces) + initial hydrate.
...
export function useRecentWorkspaces(): RecentWorkspace[] {
  const [list, setList] = useState<RecentWorkspace[]>([]);
  useEffect(() => {
    let disposed = false;
    const refresh = () =>
      void listRecentWorkspaces().then((l) => {
        if (!disposed) setList(l);
      });
    refresh();
    const un = coreStoreDeps.onDataChange((key) => {
      if (key === "recentWorkspaces") refresh();
    });
    return () => {
      disposed = true;
      un();
