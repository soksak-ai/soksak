...
describe("idleTurnDetector", () => {
  ...
  it("N ms with no output after an output burst emits turn.ended(idle) once", () => {
    ...
    configureIdleTurnDetector({ emit: (p) => emitted.push(p), projectInfoOf: () => ({ id: "wsp-aaaaaa", root: "projA" }) });
    ...
    expect(emitted).toEqual([{ projectId: "wsp-aaaaaa", root: "projA", paneId: "tab-aaaaaa", source: "idle" }]);
  });
