...
function entries(
  ...
  raw.forEach((item, index) => {
    ...
    if (!isRecord(item)) {
      errors.push(`${itemLabel}: not an object`);
      return;
    }
...
function flatId(value: unknown, label: string, errors: string[]): value is string {
  if (!isNonEmptyString(value) || !SURFACE_ID_RE.test(value)) {
    errors.push(`${label}: id must be ^[a-z0-9][a-z0-9-]*$`);
    return false;
  }
  return true;
}
...
function declaredCommand(
  ...
  if (!isNonEmptyString(value) || !commandNames.has(value)) {
    errors.push(`${label}: command "${String(value)}" is not declared in the same contributes.commands`);
    return false;
  }
  return true;
}
...
export function parseUiSurfaces(
  ...
  const overlays = entries(
    ...
    if (typeof item.capturesInput !== "boolean") {
      errors.push("contributes.overlays.capturesInput: boolean required");
      valid = false;
    }
    if (!valid) return [];
    return [{
  ...
  for (const scope of ["screen", "pane"] as const) {
    if (
      ...
      errors.push(`contributes.overlays(scope:${scope}): "ui:overlay:${scope}" permission must be declared`);
    }
  }
  ...
  const headerActions = entries(
    ...
    if (!isNonEmptyString(item.icon)) {
      errors.push("contributes.headerActions.icon: non-blank string required");
      valid = false;
    }
  ...
  if (headerActions.length > 0 && !context.permissions.includes("ui:titlebar")) {
    errors.push('contributes.headerActions: "ui:titlebar" permission must be declared');
  }
  if (headerActions.length > 0 && !context.permissions.includes("commands")) {
    errors.push('contributes.headerActions: "commands" permission must be declared');
  }
  ...
  if (statusItems.length > 0 && !context.permissions.includes("ui:statusbar")) {
    errors.push('contributes.statusItems: "ui:statusbar" permission must be declared');
  }
  if (statusItems.length > 0 && !context.permissions.includes("commands")) {
    errors.push('contributes.statusItems: "commands" permission must be declared');
  }
