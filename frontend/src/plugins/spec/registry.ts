...
async function verifyEd25519(payload: Uint8Array, signature: string, key: RegistryPublicKey): Promise<boolean> {
  try {
    const imported = await crypto.subtle.importKey(
      "raw",
      base64Bytes(key.value) as BufferSource,
      { name: "Ed25519" },
      false,
      ["verify"],
    );
    return await crypto.subtle.verify(
      { name: "Ed25519" },
      imported,
      base64Bytes(signature) as BufferSource,
...
export function resolveRegistryDependency(
  ...
  // range exists only in a plugin/kit author's intent declaration. Without it (sidecar dependency)
