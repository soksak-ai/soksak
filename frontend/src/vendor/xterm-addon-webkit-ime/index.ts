...
//
//   beforeinput(insertText U+B300)  ← addon can record echo expectation here
//   onData(U+B300)                  ← xterm textarea poll echoes the char
//   input(insertText U+B300)        ← addon processes the composition
...
//     insertText. xterm echoes it via onData before the previous syllable flushes,
//     reversing the order (U+C774 U+B300 U+B85C -> U+B300 U+C774 U+B85C U+B300 U+B85C).
//     beforeinput records the single Hangul char; shouldSkip drops the matching onData
//     exactly once.
//
...
//     producing a U+AC00 ? U+AC00 tail. The flushed syllable is recorded and the
...
export class WebkitImeAddon implements ITerminalAddon {
  ...
  // True only while _flush() runs inside _onKeydown, so the flush can tell it must
  ...
  public shouldSkip(data: string): boolean {
    ...
    // next Backspace is delivered to the pty normally, so there is no one-shot state.
    ...
    // is delivered to the pty via _flush() -> onData, never through this path.
   ...
   * before the pending syllable (U+C790 + "." → "." U+C790; U+D558 + "?" → "?" U+D558).
   * Flushing here, on the same onData path that writes the char, guarantees correct order.
  ...
  private _customKey = (ev: KeyboardEvent): boolean => {
    ...
    // previous syllable: U+D55C U+AE00 + Enter splits into U+D55C ⏎ U+AE00 ⏎. Block xterm
    // here so only _onKeydown emits the terminator.
    ...
    // Backspace is entirely the IME's. Return false so xterm never
  ...
  private _onKeydown = (e: KeyboardEvent): void => {
    ...
    // was lost. Symptoms: the 5-syllable word U+C548 U+B155 U+D558 U+C138 U+C694 + Enter
    // sent only the first four syllables and left U+C694
    ...
    // e.g. Shift+U+3145 → U+3146 for the double consonant in U+C788. Without this, the
    // Shift keydown falls into the "any other key flushes" branch below and prematurely
    // commits the pending syllable: typing U+C788 (U+C774 + U+3146) flushed U+C774 on the
    // Shift press, then composed U+C788 fresh — producing U+C774 U+C788 (likewise
    // U+AC14 → U+AC00 U+AC14, U+D588 → U+D574 U+D588). Modifier-only keys must be ignored
    // entirely here.
    if (
      e.key === "Shift" ||
      e.key === "Control" ||
  ...
  private _onInput = (e: InputEvent): void => {
    ...
    // NON-STANDARD: composition update (U+314E -> U+D558 -> U+D55C). Intercept + preview.
