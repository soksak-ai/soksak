...
describe("i18n technical writing", () => {
  ...
  it("keeps each localized message below 500 characters", () => {
    const failures = [
      ...Object.entries(ko).map(([key, value]) => ["ko", key, value] as const),
      ...Object.entries(en).map(([key, value]) => ["en", key, value] as const),
    ]
      .filter(([, , value]) => value.length > 500)
      .map(([language, key, value]) => `${language} ${key}: ${value.length}`);
    expect(failures).toEqual([]);
  });
});
