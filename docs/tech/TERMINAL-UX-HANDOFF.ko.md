...
종료나 부모 없는 사이드카 0의 증거로 볼 수 없다. 의도한 parent와 종료 뒤
프로세스 상태를 RED→GREEN 테스트로 증명할 때까지 소유권·수명주기 결함으로
추적한다. frozen v8 프로세스와 자식은 control command로 조사하거나
수정하지 않았다.
프로세스 시작 시각도 확인했다. PTY 62680은 09:21:29에 시작했고 현재 개발
앱 55409는 10:30:42에 시작했으며, Alacritty 4286은 11:08:20에 그 앱의
자식으로 시작했다. 따라서 현재 앱은 이미 실행 중이던 PTY를 재사용했다.
이 `PPID=1`은 이번 실행이 PTY를 분리했다는 증거가 아니라 이전 실행이
비정상 종료되어 부모를 잃었다는 증거다. 그래도 새 identity에서 시작·정상
종료·재시작을 통제하는 테스트로 수명주기를 GREEN으로 증명해야 한다.
...
응답하지 않으면 pane을 `live`로 유지하면 안 됩니다. 이 경로는 이벤트 기반이며 상태 조회나 타이머로
복구하지 않습니다. 서비스 테스트와 Core 패키지 테스트가 통과했습니다.
