...
모든 표면을 숨기면 캡처할 픽셀이 없는 대상이 비었습니다. 이것이
host tab ancestor의 `data-surface-visible` 로 공개되고 compositor가 Plugin 소유의 intrinsic
`data-native-visible` 과 결합합니다. 내용은 계속 보이는데 live surface 는 그렇지 않을 때,
`ParkedPicture` 가 live surface 가 돌아올 때까지 마지막으로 적용된 픽셀을 같은 슬롯에 유지합니다.
어느 쪽도 상대 선언을 다시 쓰지 않습니다.
