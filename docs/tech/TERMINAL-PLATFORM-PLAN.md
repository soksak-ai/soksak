...
| `soksak-spec-plugin-terminal` | `0.0.1` | Observable terminal-plugin lifecycle, presentation, input, commands, status and exposed nodes |
| `soksak-kit-sidecar-terminal` | `0.0.1` | One shared PTY observation, recovery service and checkpoint runtime for all six engine providers |
| `soksak-kit-plugin-terminal` | `0.0.1` | Shared terminal-plugin implementation that defines no standard and selects no engine |
...
`{ id, requirement }`. Current component requirements are exact `0.0.1`; a bounded range requires
cross-version evidence. Package dependencies remain separate and use exact remote commits or
