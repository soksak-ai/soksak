...
| browser view 의 페이지와 history | 예 | 재부착에 주소, navigation history, scroll 위치가 필요하며 어느 것도 유도되지 않습니다. 페이지 자체는 이 애플리케이션 창 안의 view 에 그려지며 그 창보다 오래 살 수 없으므로, 소유자는 상태를 들 뿐 renderer 를 들지 않습니다 (S1-2) |
...
그래서 browser session 의 소유자는 browser 플러그인이고 shell session 의 소유자는 PTY 사이드카입니다.
둘 다 같은 질문에 답합니다. 소유자가 어디서 도는지는 코어가 묻는 것이 아닙니다. 유닛은 자기 소켓으로
답하고 플러그인은 renderer 에서 답하며 명령은 같습니다. 호출자는 명령이 어디서 도는지 알 수 없고,
그것이 명령 registry 를 하나 두는 이유입니다.
...
소유한 프로세스의 것이므로 이 애플리케이션 밖의 어떤 소유자도 그것을 들 수 없습니다. browser
session 은 주소와 history 와 scroll 위치가 새 renderer 에 적용된 형태로 돌아오며, 페이지는 이어지는
것이 아니라 다시 로드됩니다.
