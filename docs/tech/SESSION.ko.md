...
그래서 restore 는 화면을 돌려주고, 그 화면을 그린 프로그램은 결코 돌려주지 않습니다. 소유자는 화면을
온전히 저장합니다. 전체 화면 프로그램이 그린 alternate screen, 그 아래 얼어 있던 primary, 커서, 색,
mode 전부입니다.
...
상태는 온전히 저장하고 flatten 은 제시 시점에 적용합니다. 그 시점에 판정 근거가 알려져 있기
때문입니다. 화면 뒤에 프로세스가 있는지입니다. 있으면 같은 저장 형태가 화면을 살아 있는 채로
복원하며, 그 경우가 [`COMPONENT-HANDOFF.md`](COMPONENT-HANDOFF.md) 의 프로세스 교체입니다.
