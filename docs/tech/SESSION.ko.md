...
때 그 기록을 제거하고, 시작 시에 코어의 index 가 명시하지 않는 모든 기록을 제거합니다.

시작 시 정리가 유일한 청소입니다. 청소하지 않는 소유자는 지금까지 실행된 모든 session 만큼 저장소를
불립니다.
...
| index 가 명시하지 않는 기록은 시작 시 제거된다 | 소유자별 저장소 테스트 |
