...
| 작업 | Session | 근거 |
| --- | --- | --- |
| pty 아래에서 실행 중인 shell | 예 | view 없이 프로세스가 계속됩니다. 재부착에 scrollback 과 작업 디렉터리가 필요합니다 |
| browser view 의 페이지와 history | 예 | 재부착에 navigation history 와 scroll 위치가 필요합니다. 어떤 view 도 표시하지 않는 동안 소유자가 페이지 로딩을 유지합니다 |
| 렌더된 터미널 화면 | 아니오 | shell 출력에서 유도됩니다. 여기서 작업은 shell session 입니다 |
...
| --- | --- | --- |
| Shell | 작업 디렉터리, shell 을 시작한 environment, scrollback, 화면, 종료 상태 | pty file descriptor, 자식 프로세스 id, subscriber 로의 연결 |
...
### S6-2. session 이 부착된 상태에서 소유자가 재시작할 때
