...
| `soksak-browser-kit` | `soksak-kit-browser-chassis` | kit — unified with the unit grammar (§1.4a: kind-first + part name; intermediate `-shell` burned on terminal-shell collision); registrar moved `~/ai/cli/` → `~/.soksak-dev/kits/` |
