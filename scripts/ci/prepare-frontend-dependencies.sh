...
if [ "${1:-}" != "--locked" ]; then
  ...
  case "$(uname -s)" in
    ...
    MINGW*|MSYS*|CYGWIN*)
      ...
      exec powershell.exe -NoProfile -Command '$owner=[Threading.Mutex]::new($false,"Global\soksak-frontend-dependencies-owner");$owner.WaitOne()|Out-Null;try{Set-Location $env:SOKSAK_PREPARE_FRONTEND;pnpm install --frozen-lockfile;exit $LASTEXITCODE}finally{$owner.ReleaseMutex()}'
      ;;
