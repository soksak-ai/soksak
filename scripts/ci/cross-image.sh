...
case "$zig_version" in [0-9]*.[0-9]*.[0-9]*) ;; *) echo "invalid Zig version: $zig_version" >&2; exit 1 ;; esac
dockerfile=$root/build/docker/Dockerfile.cross
definition=$(printf '%s\n' "$go_version" "$zig_version" "$(shasum -a 256 "$dockerfile" | awk '{print $1}')" | shasum -a 256 | awk '{print $1}')
