...
func encodeNativeFrame(frame nativeFrame, scale float64, rect Rect) ([]byte, error) {
	if frame.Width < 1 || frame.Height < 1 || frame.Stride < frame.Width*4 || len(frame.BGRA) < frame.Stride*frame.Height {
		return nil, i18n.Errorf("wails.capture.invalidFrame", map[string]string{"width": strconv.Itoa(frame.Width), "height": strconv.Itoa(frame.Height), "stride": strconv.Itoa(frame.Stride), "bytes": strconv.Itoa(len(frame.BGRA))})
	}
	...
	if encoded.Len() == 0 {
		return nil, i18n.Errorf("wails.capture.noPNG", nil)
	}
...
func captureCrop(width, height int, scale float64, rect Rect) (pixelRect, error) {
	if width < 1 || height < 1 || scale <= 0 || math.IsNaN(scale) || math.IsInf(scale, 0) {
		return pixelRect{}, i18n.Errorf("wails.capture.invalidExtent", nil)
	}
	if rect == Whole {
		return pixelRect{Width: width, Height: height}, nil
	}
	if rect.Width <= 0 || rect.Height <= 0 {
		return pixelRect{}, i18n.Errorf("wails.capture.invalidRegion", nil)
	}
	...
	if x1 <= x0 || y1 <= y0 {
		return pixelRect{}, i18n.Errorf("wails.capture.emptyRegion", nil)
	}
