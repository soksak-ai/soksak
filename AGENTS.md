...
### 3-3a. Test the seam, not one side of it

A test that starts where the change starts does not cover the join above it. Both sides can be
correct while the join is wrong.

Measured 2026-09-04: a test for a session index started one layer below the component that
constructed the writer. The test passed while that component never passed the writer. The defect
was in the untested join.

Write the test at the outermost point where the change is observable. When that requires a fake of
another component's wire, the fake is part of the work: a fake that answers the wire incorrectly
reproduces the same symptom as the defect.
...
A value recorded in two files must be changed in both. The second file is found by whatever fails.
Do not add a second copy. Where one exists, replace its assertion with a rule about the first.

Measured 2026-09-04, one kit release required three edits:

- A test asserting the literal version in `package.json`. It fails on every release and verifies
  nothing about the release. Assert the format and that the sibling files agree.
- A manifest test asserting a dependency's literal version. Every upstream release then required an
  edit in two repositories.
- A manually maintained list of the files a release packs. A source file added and not listed was
  excluded from the package; the build passed and the release verified, and the omission appeared as
  a missing module in a consumer's typecheck two repositories away.

A manually maintained list that must match reality requires a test comparing the two.
