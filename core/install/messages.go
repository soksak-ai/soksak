...
func init() {
	i18n.Declare(map[string]i18n.Sentence{
		...
		"install.transaction.projectRequired":               {EN: "install transaction needs the declared project name", KO: "설치 트랜잭션에는 선언된 프로젝트 이름이 필요합니다"},
		"install.transaction.processMaterializationInvalid": {EN: "materialized Sidecar process is invalid: {process}", KO: "materialize된 Sidecar process가 올바르지 않습니다: {process}"},
...
// The refusals probe_binary answers a caller with.
