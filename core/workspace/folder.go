...
// folderPattern is the directory-name contract for an app-made workspace folder.
//
// A slug and nothing else: `a/b` and `../etc` would both let the argument leave
// workspaces/, and the folder is created from it without a second look.
var folderPattern = regexp.MustCompile(`^[a-z0-9][a-z0-9-]*$`)

// EnsureDir creates <identity home>/workspaces/<folder> and answers with it.
//
...
// and passing the wrong one puts a workspace root inside the app's own tree.
//
// Repeating the call is an ordinary answer, not a conflict — restore and retry
// paths reach it more than once for one workspace.
func EnsureDir(folder string, identityHome string) (string, error) {
	if identityHome == "" {
		// Joining onto "" would create workspaces/ relative to whatever directory
		// this process was started in, so the same call would land in a
		// different tree per process.
		return "", i18n.Errorf("workspace.ensureDir.noIdentityHome", nil)
	}
	if !filepath.IsAbs(identityHome) {
		...
		return "", i18n.Errorf("workspace.ensureDir.relativeIdentityHome", map[string]string{"home": identityHome})
	}
	if !folderPattern.MatchString(folder) {
		return "", i18n.Errorf("workspace.ensureDir.notASlug", map[string]string{"folder": folder})
	}

	dir := filepath.Join(identityHome, "workspaces", folder)
