// Package workspace determines what may be a workspace root, where app-made workspace
...
// Verdict reports whether a canonical path may be a workspace root.
//
...
func Verdict(canonical string, home string) error {
	if canonical == home {
		// A root-init policy (git init and the rest, once per workspace.created)
		// runs over the whole root. With the home as the root, that policy runs
		// over every file the user owns.
		return i18n.Errorf("workspace.root.home", map[string]string{"path": canonical})
	}
	if filepath.Dir(canonical) == canonical {
		...
		return i18n.Errorf("workspace.root.filesystemRoot", map[string]string{"path": canonical})
	}
	return nil
}

// ValidateRoot answers whether path may be a workspace root, and returns the
// canonical spelling of it.
//
// That returned string is the workspace's identity: duplicate checks and claims
// key on it, so two spellings of one directory must not become two workspaces.
...
func ValidateRoot(path string, userHome string) (string, error) {
	if userHome == "" {
		...
		// the home itself.
		return "", i18n.Errorf("workspace.validateRoot.noUserHome", nil)
	}
	...
	if !filepath.IsAbs(userHome) {
		...
		// would say so.
		return "", i18n.Errorf("workspace.validateRoot.relativeUserHome", map[string]string{"home": userHome})
	}
	if !filepath.IsAbs(path) {
		...
		// Two spellings of one directory then become two workspaces.
		return "", i18n.Errorf("workspace.validateRoot.relativePath", map[string]string{"path": path})
	}
	...
	if err != nil || !info.IsDir() {
		return "", i18n.Errorf("workspace.validateRoot.notDirectory", map[string]string{"path": path})
	}
